...
# ---------------------------------------------------------------------------
# Test 2 — TRAINERS constant contains all 10 trainer types
# ---------------------------------------------------------------------------

def test_trainers_constant():
    """All 10 trainer types must be present in TRAINERS."""
    from api.evaluation import TRAINERS

    expected = {
        "FedRL", "MARL", "SARL",
        "MeanField", "Gossip", "CTDE", "HierFed", "FedDistill",
        "fixed-time", "max-pressure",
    }
    assert set(TRAINERS) == expected, f"TRAINERS mismatch: {TRAINERS}"
    assert len(TRAINERS) == 10, f"Expected 10 trainers, got {len(TRAINERS)}"
