...
pipelines for all 8 RL trainers (using example weights), plus fixed-time
...
def run_baseline_campaign(
    ...
    incremental_save_name: Optional[str] = None,
) -> List[CampaignResult]:
    ...
    # Resume support: skip configs whose name is already in existing results.json
    completed_names = set()
    if incremental_save_name:
        _BACKEND_DIR = os.path.normpath(os.path.join(os.path.dirname(__file__), ".."))
        results_path = os.path.join(
            _BACKEND_DIR, "results", "campaigns", incremental_save_name, "results.json",
        )
        if os.path.exists(results_path):
            try:
                with open(results_path, "r", encoding="utf-8") as fp:
                    ...
                    completed_names = {
                        r.get("config", {}).get("name") for r in existing
                    }
                logger.info(
                    "Resuming: %d configs already in %s — will skip",
                    len(completed_names), results_path,
                )
    ...
    for i, cfg in enumerate(configs, start=1):
        if cfg.name in completed_names:
            logger.info("Campaign %d/%d: %s on %s — SKIP (already done)",
        ...
        if incremental_save_name:
            try:
                save_campaign_results([camp_result], campaign_name=incremental_save_name)
            except Exception as exc:  # noqa: BLE001
                logger.error("Incremental save failed: %s", exc)
...
def main():
    ...
    results = run_baseline_campaign(
        ...
        incremental_save_name=args.output_name,
    )

    # Results already saved incrementally; compute output_dir for summary
    _BACKEND_DIR = os.path.normpath(os.path.join(os.path.dirname(__file__), ".."))
    output_dir = os.path.join(_BACKEND_DIR, "results", "campaigns", args.output_name)
