...
When working on the SEAL Dashboard project (`PROJECT-PLAN.md`), two source directories are read-only references — never modify them, only copy from them:

| Source directory | Copy into | Agent |
|-----------------|-----------|-------|
| `SUMO-FedRL-main/seal/`, `configs/`, `example_weights/`, `netfiles.py` | `BackEnd/` | Backend agent |
| `LovableOutput/seal-traffic-flow-main/` | `FrontEnd/` | Frontend agent |

The frontend agent ports the Lovable scaffold into `FrontEnd/`, then wires it to the live API — it does **not** rebuild the UI from scratch. The backend agent copies the SEAL framework into `BackEnd/` — it does **not** modify `SUMO-FedRL-main/`.
